...
class FileLocationConverter {
  ...
  getKey(file) {
    ...
    const filename = `${sanitizedFileName}-${file.hash}${file.ext}`;
  ...
  init: (config) => {
    ...
    const upload = (file) =>
      new Promise((resolve, reject) => {
        ...
        S3.upload(
          {
            Key: converter.getKey(file), // Use custom naming logic here
          ...
          (err, data) => {
            ...
            file.url = converter.getUrl(data); // Generate the file's URL
            delete file.buffer;
            resolve();
          }
        );
